...
const connectDB = async () => {
  try {
    await mongoose.connect(
      process.env.MONGODB_URI || "mongodb://localhost:27017/ses"
    );
...
const seedData = async () => {
  try {
    ...
    if (districts.length === 0) {
      console.error("No districts found! Please create districts first.");
      process.exit(1);
    }
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      const age = Math.floor(
        (illnessDate - birthDate) / (1000 * 60 * 60 * 24 * 365.25)
      );
      ...
      const forma60 = {
        ...
        address: {
          ...
          fullAddress: `${district.name}, ${randomInt(1, 100)}-uy, ${randomInt(
            1,
            50
          )}-xonadon`,
          coordinates: {
        ...
        diagnosisDate: new Date(
          illnessDate.getTime() + randomInt(1, 7) * 24 * 60 * 60 * 1000
        ),
        primaryDiagnosis: disease,
        finalDiagnosis: disease,
        diagnosisMethod: randomElement([
          "laboratoriya",
          "klinik",
          "epidemiologik",
        ]),
        ...
        hospitalName:
          Math.random() > 0.7 ? "Nukus shahar kasalxonasi" : undefined,
    ...
    // Insert Forma60 records in batches
    console.log("Inserting Forma60 records into database...");
    const insertedForma60s = await Forma60.insertMany(forma60Records);
    console.log(
      `✓ Successfully inserted ${insertedForma60s.length} Forma60 records`
    );
    ...
    for (let i = 0; i < 1000; i++) {
      const forma60 = randomElement(insertedForma60s);
    ...
    // Insert Karta records in batches
    console.log("Inserting Karta records into database...");
    const insertedKartas = await Karta.insertMany(kartaRecords);
    console.log(
      `✓ Successfully inserted ${insertedKartas.length} Karta records`
    );

    console.log("\n✅ Data seeding completed successfully!");
    console.log(`📊 Summary:`);
    console.log(`   - Forma60 records: ${insertedForma60s.length}`);
    console.log(`   - Karta records: ${insertedKartas.length}`);
